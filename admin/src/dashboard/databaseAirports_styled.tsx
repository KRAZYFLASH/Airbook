...
import { AddButton, Header, Pagination, Modal } from '../components/UI';
...
export function DatabaseAirportsStyled() {
    ...
    // Handler untuk membuka modal tambah bandara
    ...
    // Handler untuk membuka modal edit bandara
    const handleEditAirport = (airport: DatabaseAirport) => {
        setFormData({
            name: airport.name,
            iataCode: airport.iataCode || '',
            icaoCode: airport.icaoCode || '',
            cityId: airport.city.id.toString(),
            countryId: airport.city.country.id.toString(),
            timezone: airport.timezone || 'UTC+0',
            isActive: true
        });
        setEditingAirport(airport);
        setShowModal(true);
        loadDropdownData();
        loadCitiesByCountry(airport.city.country.id);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!formData.name.trim()) {
                alert('Nama bandara harus diisi');
            ...
            const endpoint = editingAirport
                ? `http://localhost:3001/api/db-airports/${editingAirport.id}`
                : 'http://localhost:3001/api/db-airports';
            ...
            const response = await fetch(endpoint, {
                ...
                body: JSON.stringify({
                    name: formData.name,
                    iataCode: formData.iataCode || undefined,
                    icaoCode: formData.icaoCode || undefined,
            ...
            if (result.success) {
                handleCloseModal();
                loadAirports(); // Reload data
                alert(editingAirport ? 'Bandara berhasil diupdate!' : 'Bandara berhasil ditambahkan!');
            } else {
                // Show more detailed error message
                if (result.error && result.error.includes('Unique constraint failed')) {
                    if (result.error.includes('iataCode')) {
                        alert('Error: Kode IATA sudah digunakan oleh bandara lain. Gunakan kode yang berbeda.');
                    } else if (result.error.includes('icaoCode')) {
                        alert('Error: Kode ICAO sudah digunakan oleh bandara lain. Gunakan kode yang berbeda.');
                    } else {
                        alert('Error: Data sudah ada dalam database. Periksa kode IATA/ICAO.');
                    }
                } else {
    ...
    return (
        <section className="space-y-3">
            ...
            {/* Modal untuk Add/Edit Bandara */}
            {showModal && (
                <Modal
                    title={editingAirport ? "Edit Bandara" : "Tambah Bandara Baru"}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
                            ...
                            <button
                                ...
                                disabled={formLoading}
                            >
                                {formLoading ? 'Menyimpan...' : (editingAirport ? "Simpan Perubahan" : "Tambah Bandara")}
                            </button>
                        </div>
                    </form>
                </Modal>
